...
function faithful(tics){    
     if((cult.faithful.current + cult.hybrids.current)>=1){
...
function chanters(tics) {
    if (cult.chanters.current >= 1 &&  (cult.faithful.current + cult.hybrids.current) > 0) {
...
function sentinels(tics) {
    if ((cult.sentinels.current + cult.hybrids.current) >= 1 && (cult.faithful.current + cult.hybrids.current) > 0) {
        cult.sentinels.ticCounter += tics;
        if (cult.sentinels.ticCounter >= cult.sentinels.ticsNeeded) {
            cult.sentinels.ticCounter -= cult.sentinels.ticsNeeded;
...
function offlineProgress(){
    ...
    if(tics >= 75){
        for (let i = 0; i < tics; i += 75) {
            ...
            window.console.log(offlineOutput);
        }
            window.console.log(offlineOutput);
    }
